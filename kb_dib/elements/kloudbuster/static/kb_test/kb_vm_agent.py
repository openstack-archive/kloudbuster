...
def exec_command(cmd, cwd=None, op=[]):
    p = subprocess.Popen(cmd, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    (stdout, stderr) = p.communicate()
    op.append(stdout + stderr)
...
class KBA_Client(object):
    ...
    def post_processing(self, p_out):
        ...
        if self.__class__.__name__ == 'KBA_Storage_Client':
        ...
        elif self.__class__.__name__ == "KBA_Multicast_Client":
            ...
            kmap = {'pkt': 'packets_recv', 'data_loss': 'data_loss', 'drop': 'packets_dropped',
                    ...
                    'msavgOWD': 'latency', 'real_seconds': 'seconds'}  # Format/Include Keys
            try:
                #         {mbps : 1200}    (rate_Mbps,1200)   'rate_Mbps=100'  only if key in map
...
class KBA_Server(object):
    ...
    def start_multicast_listener(self, mc_addrs, multicast_ports, start_address="231.0.0.128"):
        '''Starts Listeners at second /25 (.128).
           These listeners are created when nRecievers > 1.
        '''
        startPort = 12000
...
if __name__ == "__main__":
    ...
    refresh_clock(user_data.get('ntp_clocks', '-b'))
    if user_data.get('role') == 'KB-PROXY':
        agent = KBA_Proxy()
        sys.exit(agent.start_redis_server())
    if user_data.get('role')[-6:] == 'Server':
        agent = KBA_Server(user_data)
        if user_data['role'][:-7] == 'Multicast':
            if user_data['n_id'] == 0:
        ...
        if user_data['role'][:-7] == 'HTTP':
            agent = KBA_HTTP_Client(user_data)
        elif user_data['role'][:-7] == 'Multicast':
