/*
 *	N U T T C P . C						v7.3.3-1
 *
 * Copyright(c) 2000 - 2015 Bill Fink.  All rights reserved.
 * Copyright(c) 2003 - 2015 Rob Scott.  All rights reserved.
 *
 ...
 *
 * 7.3.3-1, Michael Napolitano, 13-Apr-16
 * 	Added Delay in nwrite to allow for multiple simultaneous processes and to avoid maxing CPU.
 *  Added/Fixed nstream (-N) support for multicast.
...
acceptnewconn:
	...
	if (multicast) {
		...
		if (!trans) {
			...
			int stream_i = 1;
			for (; stream_i <= nstream; stream_i++){	//nstream support for multicast: 7.3.3-1
				if (setsockopt(fd[stream_i], IPPROTO_IP, IP_ADD_MEMBERSHIP,
			...
			if (mc_addr) {
				...
				for (; stream_i <= nstream; stream_i++){	//nstream support for multicast: 7.3.3-1
					bcopy((char *)&(user_group->sin_addr.s_addr),
					      (char *)&sinhim[stream_i].sin_addr.s_addr,
					      sizeof(struct in_addr));
				}
			...
			int stream_i = 1;
			for (; stream_i <= nstream; stream_i++){	//nstream support for multicast: 7.3.3-1
				if (setsockopt(fd[stream_i], IPPROTO_IP, IP_MULTICAST_TTL,
					       (void *)&multicast,
					       sizeof(multicast)) < 0)
