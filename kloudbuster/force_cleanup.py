...
class StorageCleaner(AbstractCleaner):
    def __init__(self, creds, resources, dryrun):
        ...
        self.cinder = client.Client('2', crd['username'], crd['password'], crd['tenant_name'], crd['auth_url'])
    ...
    def clean(self):
        ...
        try:
            ...
            if kb_detaching_volumes:
                print '    . Waiting for %d volumes to be fully detached...' % (len(kb_detaching_volumes))
                retry_count = 2
                while True:
                    retry_count -= 1
                    ...
                    if kb_detaching_volumes and not self.dryrun:
                        if retry_count:
                            print '    . VOLUME %d left to be detached, retries left=%d...' % (len(kb_detaching_volumes), retry_count)
                            time.sleep(1)
                        else:
                            print '    - VOLUME detach timeout, %d volumes left:' % (len(kb_detaching_volumes))
